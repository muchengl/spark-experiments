...
class GCLogParser:
    ...
    concurrentCycle_patttern = r"^\[(\d+\.\d+)s\]\[info\s*\]\[gc\s*\] GC\((\d+)\) (Concurrent Cycle)( (\d+)M->(\d+)M\((\d+)M\))?( (\d+\.\d+)ms)?$"
...
def parse(logpath,logname):
    a = GCLogParser(logpath)
    a.heapUsageAnalysis()
    
    print(f"Total Concurrent Time  : {a.totalTimeConcurrentCycle()}s")
    print(f"Total Pause Time       : {a.totalTimePauseTime()}s")
    print(f"Total Pause Young Time : {a.totalTimePauseYoung()}s")
    print(f"Total Pause Full Time  : {a.totalTimePauseFull()}s")
    print(f"Total Pause Other Time : {a.totalTimeOtherPause()}s")

    items = logname.split("_")
    items_3 = items[2].split(".")
    write_csv(logname,items[0],items[1],items_3[0],[a.totalTimePauseTime(),a.totalTimePauseYoung(),a.totalTimePauseFull(),a.totalTimeOtherPause(),a.totalTimeConcurrentCycle(),sum(a.heapUsage()),a.avgHeapUsage(),a.minHeapUsage(),a.maxHeapUsage()])

def write_csv(filename,instance,memory,filesize,r):
    with open("./data/parser-result.csv", 'a') as file:
        ...
        # 2 memory 
        # 3 filesize 
        # 4 totalTimePauseTime 
        # 5 totalTimePauseYoung 
        # 6 totalTimePauseFull 
        # 7 totalTimeOtherPause 
        # 8 totalTimeConcurrentCycle 
        # 9 totalHeapUsage 
        # 10 avgHeapUsage 
        # 11 minHeapUsage 
        # 12 maxHeapUsage
        if filesize=='wiki':
            filesize="18.53G"
        row = [filename,instance,memory,filesize]
        row.extend(r)
